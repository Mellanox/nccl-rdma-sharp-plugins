...
if [ -z "${NFS_WORKSPACE}" ]; then
    echo "ERROR: NFS_WORKSPACE is not defined"
    echo "FAIL"
    exit 1
fi

if [ -z "${HPCX_SHARP_DIR}" ]; then
...
if [ -f "${SHARP_CONF}/sharp_am_node.txt" ]; then
...
if [ "${SHARP_MANAGER_ACTION}" != "stop" ]; then
    check_opensm_status
    check_opensm_conf
fi
...
if [ "${SHARP_MANAGER_ACTION}" != "stop" ] && [ "${VERIFY_SHARP_ENABLE}" -eq 1 ]; then
    verify_sharp
fi
