    ...
    if (bytes == -1) {
      ...
      if (errno != EINTR && errno != EWOULDBLOCK && errno != EAGAIN) {
        WARN("socketProgressOpt: Call to %s %s failed : %s", (op == NCCL_SOCKET_RECV ? "recv from" : "send to"),
             ncclSocketToString(&sock->addr, line, 1), strerror(errno));
...
static ncclResult_t socketProgress(int op, struct ncclSocket* sock, void* ptr, int size, int* offset, int* pclosed) {
  int closed;
...
static ncclResult_t socketWait(int op, struct ncclSocket* sock, void* ptr, int size, int* offset) {
  while (*offset < size)
    NCCLCHECK(socketProgress(op, sock, ptr, size, offset, NULL));
  return ncclSuccess;
}
  ...
  *found = 0;
  ...
  for (interface = interfaces; interface && !*found; interface = interface->ifa_next) {
    ...
    TRACE(NCCL_INIT|NCCL_NET,"NET : Found interface %s:%s in the same subnet as remote address %s",
          interface->ifa_name, ncclSocketToString(localAddr, line, 1), ncclSocketToString(remoteAddr, line_a, 1));
    *found = 1;
  }
...
ncclResult_t ncclFindInterfaces(char* ifNames, union ncclSocketAddress *ifAddrs, int ifNameMaxSize, int maxIfs,
  ...
  *nIfs = 0;
  if (env && strlen(env) > 1) {
    ...
    // Finally look for docker, lo, then virbr.
...
#ifdef ENABLE_TRACE
  char line[SOCKET_NAME_MAXLEN+1];
  TRACE(NCCL_INIT|NCCL_NET,"Listening on socket %s", ncclSocketToString(&sock->addr, line, 1));
#endif
...
static void socketResetAccept(struct ncclSocket* sock) {
  ...
  INFO(NCCL_NET|NCCL_INIT, "socketFinalizeAccept: didn't receive a valid magic from %s",
       ncclSocketToString(&sock->addr, line, 1));
...
static ncclResult_t socketFinalizeAccept(struct ncclSocket* sock) {
  ...
  if (sock->asyncFlag == 0) {
    ...
    NCCLCHECK(socketProgress(NCCL_SOCKET_RECV, sock, sock->finalizeBuffer, sizeof(type), &received, NULL));
  ...
  if (type != sock->type) {
    WARN("socketFinalizeAccept from %s: wrong type %d != %d", ncclSocketToString(&sock->addr, line, 1), type, sock->type);
...
static ncclResult_t socketConnectCheck(struct ncclSocket* sock, int errCode, const char funcName[]) {
  ...
  if (errCode == 0) {
    ...
    if (sock->customRetry == 0) {
      if (sock->errorRetries++ == ncclParamRetryCnt()) {
        ...
        WARN("%s: connect to %s returned %s, exceeded error retry count after %d attempts",
             funcName, ncclSocketToString(&sock->addr, line, 1), strerror(errCode), sock->errorRetries);
        return ncclRemoteError;
      }
      ...
      INFO(NCCL_NET|NCCL_INIT, "%s: connect to %s returned %s, retrying (%d/%ld) after sleep for %u msec",
           funcName, ncclSocketToString(&sock->addr, line, 1), strerror(errCode),
    ...
    sock->state = ncclSocketStateError;
    WARN("%s: connect to %s failed : %s", funcName, ncclSocketToString(&sock->addr, line, 1), strerror(errCode));
    return ncclSystemError;
  }
  return ncclSuccess;
}
...
static ncclResult_t socketPollConnect(struct ncclSocket* sock) {
  ...
  if (ret == 0 || (ret < 0 && errno == EINTR)) {
    ...
    WARN("socketPollConnect to %s failed with error %s", ncclSocketToString(&sock->addr, line, 1), strerror(errno));
    return ncclSystemError;
  }
...
static ncclResult_t socketFinalizeConnect(struct ncclSocket* sock) {
  ...
  if (sock->asyncFlag == 0) {
    ...
    if (sock->finalizeCounter < sizeof(sock->magic)) {
      ...
      NCCLCHECK(socketProgress(NCCL_SOCKET_SEND, sock, &sock->magic, sizeof(sock->magic), &sent, NULL));
      sock->finalizeCounter = sent;
      if (sent < sizeof(sock->magic)) return ncclSuccess;
    }
    sent = sock->finalizeCounter - sizeof(sock->magic);
    NCCLCHECK(socketProgress(NCCL_SOCKET_SEND, sock, &sock->type, sizeof(sock->type), &sent, NULL));
  ...
  if (addr) {
    ...
    if (family != AF_INET && family != AF_INET6) {
      ...
      WARN("ncclSocketInit: connecting to address %s with family %d is neither AF_INET(%d) nor AF_INET6(%d)",
          ncclSocketToString(&sock->addr, line, 1), family, AF_INET, AF_INET6);
      ret = ncclInternalError;
      goto exit;
    }
...
ncclResult_t ncclSocketSendRecv(struct ncclSocket* sendSock, void* sendPtr, int sendSize, struct ncclSocket* recvSock, void* recvPtr, int recvSize) {
  ...
  while (sendOffset < sendSize || recvOffset < recvSize) {
    if (sendOffset < sendSize) NCCLCHECK(socketProgress(NCCL_SOCKET_SEND, sendSock, sendPtr, sendSize, &sendOffset, NULL));
    if (recvOffset < recvSize) NCCLCHECK(socketProgress(NCCL_SOCKET_RECV, recvSock, recvPtr, recvSize, &recvOffset, NULL));
  }
  return ncclSuccess;
}
