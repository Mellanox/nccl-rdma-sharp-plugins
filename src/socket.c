  ...
  do {
    ...
    if (bytes == -1) {
      if (errno != EINTR && errno != EWOULDBLOCK && errno != EAGAIN) {
        WARN("socketProgressOpt: Call to recv from %s failed : %s", ncclSocketToString(&sock->addr, line, 1), strerror(errno));
...
static ncclResult_t socketStartConnect(struct ncclSocket* sock) {
  ...
  if (ret == 0) {
    ...
    char line[SOCKET_NAME_MAXLEN+1];
    sock->state = ncclSocketStateError;
    WARN("socketStartConnect: Connect to %s failed : %s", ncclSocketToString(&sock->addr, line, 1), strerror(errno));
    return ncclSystemError;
  }
}
